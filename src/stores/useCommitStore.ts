  ...
  async function fetchCommits(username: string, repo: string, toPage?: number) {
    ...
    try {
      const url = `https://api.github.com/repos/${encodeURIComponent(
        u
      )}/${encodeURIComponent(r)}/commits?per_page=${perPage.value}&page=${
        page.value
      }`;
      const res = await fetch(url);

      if (res.status === 403)
        throw new Error("API rate limit exceeded (403). Try again later.");
      if (res.status === 404)
        throw new Error("Repository or user not found (404).");
