...
<template>
  <div class="repo-view">
    ...
    <div class="repo-grid" v-else>
      <article v-for="repo in repos" :key="repo.id" class="card glass repo-card">
...
.star-count {
  display: inline-flex;
  align-items: center;
  gap: 0.25rem; /* relative to root font size */
  color: #f5c518;
  font-size: 1rem;
}
...
.repo-view { padding: 4rem 2rem; max-width: 1200px; margin: 0 auto; color: var(--text); }
.title { font-size: 2rem; font-weight: 700; margin-bottom: 2rem; text-align: center; }
.repo-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1.5rem; }
...
.btn { background: var(--bg-elev); color: var(--text); border: 1px solid var(--border); padding: .45rem .7rem; border-radius: .5rem; text-decoration: none; }
