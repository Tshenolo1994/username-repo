...
// Initial fetch + reset when route changes
watchEffect(() => {
...
function toggleFave(c: any) {
  // minimal shape stored for favourites panel
  faves.toggle({
    sha: c.sha,
...
<template>
  <section class="commit-page">
    <!-- Top bar -->
    <div class="bar">
      <button class="back" @click="backToRepos">← Back</button>
    ...
    <!-- Global search (navigates to /repos/:username) -->
    <SearchBar />
    ...
    <ul v-else-if="store.sorted.length" class="commit-list">
      <li v-for="c in store.sorted" :key="c.sha" class="card glass">
        <div class="row">
          <!-- Main (clickable) -->
          <button class="main" @click="viewDetails(c.sha)" aria-label="Open commit details">
  ...
  @click.stop.prevent="toggleFave(c)"   <!-- add .stop + .prevent -->
  :aria-label="faves.isFaved(c.sha) ? 'Remove favourite' : 'Add favourite'"
...
>
  <Star :size="18" />
            ...
            >
              {{ c.sha.slice(0, 7) }} <ExternalLink :size="14" />
            </a>
          </div>
        </div>
      </li>
...
.commit-list { display: grid; gap: 12px; margin: 0; padding: 0; list-style: none; }
...
.main { background: transparent; border: none; color: inherit; text-align: left; cursor: pointer; padding: 0; }
