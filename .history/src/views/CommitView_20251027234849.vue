      ...
      <div v-for="c in store.sorted" :key="c.sha" class="card glass">
        ...
        <div class="card-actions">
          <button
            type="button"
            class="icon-btn"
            :class="{ active: faves.isFaved(`${repoFullName}#${c.sha}`) }"
          ...
          <a class="sha-link" :href="c.html_url" target="_blank" rel="noreferrer" title="View on GitHub">
            {{ c.sha.slice(0, 7) }}
...
.card-header { width:100%; text-align:left; background:transparent; border:none; color:inherit; cursor:pointer; padding:0; }
