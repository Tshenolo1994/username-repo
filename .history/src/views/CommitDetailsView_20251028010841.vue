  ...
  <section class="commit-page">
    ...
    <section v-else class="commit-grid">
      <div v-for="c in store.sorted" :key="c.sha" class="card glass">
        ...
        <!-- actions must sit above header for clicks -->
...
.card { position:relative; padding:14px; border-radius:12px; }
...
.card-header{
  width:100%; text-align:left; background:transparent; border:none; color:inherit; cursor:pointer; padding:0;
  position:relative; z-index:1;   /* header sits below actions */
}
...
.card-actions{
  ...
  position:relative; z-index:2;   /* ensure clickable above header */
  pointer-events:auto;
}
