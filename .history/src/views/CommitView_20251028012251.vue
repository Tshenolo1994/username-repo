...
function viewDetails(sha: string) {
  router.push({ name: "commitDetails", params: { username: username.value, repo: repo.value, sha } });
}
...
<template>
  <section class="commit-page">
    ...
    <section v-else class="commit-grid">
      <article v-for="c in store.sorted" :key="c.sha" class="card glass">
...
<style scoped>
.commit-page{display:grid;gap:16px;padding:16px;max-width:1000px;margin:0 auto;color:var(--text)}
.bar{display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:12px}
.back{justify-self:start;background:var(--bg-elev);color:var(--text);border:1px solid var(--border);padding:.45rem .7rem;border-radius:.5rem;cursor:pointer}
.title{justify-self:center;font-weight:800;font-size:1.2rem}
.sort{justify-self:end;display:inline-flex;gap:.5rem;align-items:center}
...
.commit-grid{display:grid;gap:12px}
...
.meta{margin:0;color:var(--muted);font-size:13px;display:inline-flex;align-items:center;gap:8px}
.head-right{flex-shrink:0}
.btn.details{background:var(--bg-elev);color:var(--text);border:1px solid var(--border);padding:.4rem .6rem;border-radius:.5rem;cursor:pointer}
.btn.details:hover{border-color:var(--link)}

.card-actions{margin-top:10px;display:inline-flex;align-items:center;gap:8px}
.icon-btn{display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:8px;border:1px solid var(--border);background:var(--bg-elev);color:var(--text);cursor:pointer;transition:border-color .15s,color .15s,background .15s}
.icon-btn:hover{border-color:var(--link)}
.icon-btn.active{color:#f5c518;border-color:#f5c518}

.sha-link{display:inline-flex;align-items:center;gap:6px;color:var(--link);text-decoration:none;font-family:ui-monospace,monospace;border:1px solid var(--border);background:var(--bg-elev);padding:.25rem .5rem;border-radius:6px}
@media (max-width:640px){
  .card-top{flex-direction:column;align-items:stretch}
  .head-right{align-self:flex-end}
}
</style>