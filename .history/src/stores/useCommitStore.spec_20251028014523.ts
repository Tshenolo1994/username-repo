...
describe("useCommitStore", () => {
  ...
  it("fetches first page and sets hasNext based on perPage", async () => {
    ...
    // Return exactly perPage commits to imply a likely next page
    ...
    expect(store.hasNext).toBe(true); // length === perPage → probably next page
  });
  ...
  it("nextPage loads page 2 and sets hasPrev", async () => {
    ...
    // page 2 (return fewer than perPage to indicate no further page)
    // @ts-expect-error mock
    global.fetch.mockResolvedValueOnce({
      ok: true,
  ...
  it("sort switching updates order", async () => {
    ...
    // out-of-order dates to test sorting
    // @ts-expect-error mock
    global.fetch.mockResolvedValueOnce({
      ok: true,
      json: async () => [
